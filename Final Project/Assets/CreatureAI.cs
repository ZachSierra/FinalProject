...
public class CreatureAI : MonoBehaviour
{
    ...
    public GameObject GetTarget(){
        //are we close enough?
        if(Vector3.Distance(myCreature.transform.position,target.transform.position) > sightDistance){
            return null;
        }

        //is vision blocked by a wall?
        RaycastHit2D hit = Physics2D.Linecast(myCreature.transform.position, target.transform.position,obstacles);
    ...
    public void GetTargetMoveCommand(ref List<Vector2> path){
        GetMoveCommand(target.transform.position, ref path);
